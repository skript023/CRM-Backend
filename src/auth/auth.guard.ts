...
export class AuthGuard implements CanActivate 
{
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> 
    {
        ...
        if (!token) 
        {
            throw new UnauthorizedException('MWG01 - You are not logged in, please login!');
        }

        if (!user)
        {
            throw new UnauthorizedException('MWG02 - Your login data is invalid, please login!');
        }
        ...
        try 
        {
            ...
            if (data._state != user._id) throw new UnauthorizedException('MWG03 - Your login data is invalid, please login!');

            const role_exist = roles.some(value => value === role?.name);

            const action_exist: boolean = role?.access[actions];

            if (!role_exist || !action_exist) throw new UnauthorizedException('MWG03 - You are not have authority to access this page!')
